...
class Env:
    ...
    def step(self, action, flow_traffic, queue_length_select, a_delay, a_pkt_loss):
        ...
        reward, r_delay, r_pkt_loss = self.get_reward(state, action_w, queue_length_select, a_delay, a_pkt_loss)
        return state, reward, r_delay, r_pkt_loss
        ...
        for path in range(len(optimal_path)):
            for i in range(len(optimal_path[path]) - 1):
                ...
                if sum_bandwidth > self.bandwidth[optimal_path[path][i]][optimal_path[path][i + 1]]:
                    ...
                    # assume each flow has 20 packets
                    # queue length is 20*(#flows), in order to make sure there is no packet loss
                    path_latency_que[path] += 1512 * 8 * 20 * queue_length_select / \
