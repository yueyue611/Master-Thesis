...
for ft in range(mode_select):
    if mode_select == len_traffic_load:
        ...
        flow_traffic = [flows_tl[-2][i][2] for i in range(len(flows_tl[-2]))]
    ...
    for ep in range(total_episodes):
        ...
        for step in range(total_steps):
            ...
            state, reward = env.step(action, flow_traffic, queue_length_select, a_delay, a_pkt_loss)
