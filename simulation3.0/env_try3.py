...
class Env:
    ...
    def step(self, action, flow_traffic, queue_length_select):
        action_w = np.array(action).reshape((self.total_switches, self.total_switches))
        state = self.get_state(action_w, flow_traffic)
        reward = self.get_reward(state, action_w, queue_length_select)
        return state, reward

    # calculates reward
    def get_reward(self, state, action, queue_length_select):
        ...
        for path in range(len(optimal_path)):
            for i in range(len(optimal_path[path]) - 1):
                ...
                if sum_bandwidth > self.bandwidth[optimal_path[path][i]][optimal_path[path][i + 1]]:
                    ...
                    # assume each flow has 30 packets
                    # queue length is 30*(#flows), in order to make sure there is no packet loss
                    path_latency_que[path] += 1512 * 8 * 30 * queue_length_select / \
