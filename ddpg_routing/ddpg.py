...
def policy(actor_model, state, noise, weights_original, indicator, exploration_rate=1.0):
    if action_mode == 2:
        weights = np.array(weights_original+weights_original).reshape(1, nodes ** 2 * action_mode)
    else:
        weights = np.array(weights_original).reshape(1, nodes ** 2 * action_mode)
...
def main():
    ...
    random_flow_change = Config.random_flow_change
    changed_flows_num = Config.changed_flows_num
    new_flows = env.new_flows(mode_flow_change, random_flow_change, changed_flows_num)
    ...
    for ex in range(11, experiment + 11):
        ...
        # to store average reward history of last few episodes !!!!!
        avg_reward_list2 = [[] for j in range(mode_select)]
        avg_r_delay_list2 = [[] for j in range(mode_select)]
        avg_r_pkt_loss_list2 = [[] for j in range(mode_select)]
        avg_avg_delay_list2 = [[] for j in range(mode_select)]
        ...
        # !!!!!!
        ep_converged2 = [0 for k in range(mode_select)]
        reward_converged2 = [0 for k in range(mode_select)]
            ...
            for ep in range(total_episodes):
                ...
                if ep == N:
                    if mode_flow_change != "NONE":
                        ...
                        """
                        action = np.array(action).reshape((nodes * action_mode, nodes))
                ...
                for step in range(total_steps):
                    ...
                    # random selection
                    paths_random = [all_paths[i][np.random.randint(0, len(all_paths[i]))]
                                    for i in range(0, env.total_flows)]
                ...
                if mode_flow_change == "NONE":
                    ...
                    ep_min = N
                for i in range(0, min(ep_min, ep + 1)):
                    cost += (ep_reward_list[ft][-1 - i] - avg_reward) ** 2
                RMSD = math.sqrt(cost / min(ep_min, ep + 1))
                # print("RMSD: ", RMSD)
                if RMSD < 0.001 and ep_converged[ft] == 0 and ep >= ep_min:
                    ep_converged[ft] = ep + 1
                    reward_converged[ft] = ep_reward_list[ft][-1]

                # mean reward of last 20 episodes !!!!!!!
                avg_reward2 = np.mean(ep_reward_list[ft][-20:])
                avg_reward_list2[ft].append(avg_reward2)

                avg_r_delay2 = np.mean(ep_r_delay_list[ft][-20:])
                avg_r_delay_list2[ft].append(avg_r_delay2)

                avg_avg_delay2 = np.mean(ep_avg_delay_list[ft][-20:])
                avg_avg_delay_list2[ft].append(avg_avg_delay2)

                avg_r_pkt_loss2 = np.mean(ep_r_pkt_loss_list[ft][-20:])
                avg_r_pkt_loss_list2[ft].append(avg_r_pkt_loss2)

                # to see whether the total reward has converged, sliding window size = 20 !!!!!!
                cost2 = 0
                if mode_flow_change == "NONE":
                    ep_min2 = 20
                else:
                    ep_min2 = N
                for i in range(0, min(ep_min2, ep + 1)):
                    cost2 += (ep_reward_list[ft][-1 - i] - avg_reward2) ** 2
                RMSD2 = math.sqrt(cost2 / min(ep_min2, ep + 1))
                if RMSD2 < 0.001 and ep_converged2[ft] == 0 and ep >= ep_min2:
                    ep_converged2[ft] = ep + 1
                    reward_converged2[ft] = ep_reward_list[ft][-1]
        ...
        df.to_csv("/home/tud/Github/Master-Thesis/ddpg_routing/csv/{}/{}/No.{}, {}, {}, {}, {}, {}.csv"
                  .format(folder, noise_mode, ex, total_episodes, total_steps, noise_mode, mode, mode_flow_change),
                  header=False, index=False)

        df = pd.DataFrame(ep_r_delay_list)
        df.to_csv("/home/tud/Github/Master-Thesis/ddpg_routing/csv/{}/{}/r_delay: No.{}, {}, {}, {}, {}, {}.csv"
                  .format(folder, noise_mode, ex, total_episodes, total_steps, noise_mode, mode, mode_flow_change),
                  header=False, index=False)

        df = pd.DataFrame(ep_avg_delay_list)
        df.to_csv("/home/tud/Github/Master-Thesis/ddpg_routing/csv/{}/{}/avg_delay: No.{}, {}, {}, {}, {}, {}.csv"
                  .format(folder, noise_mode, ex, total_episodes, total_steps, noise_mode, mode, mode_flow_change),
                  header=False, index=False)

        df = pd.DataFrame(ep_r_pkt_loss_list)
        df.to_csv("/home/tud/Github/Master-Thesis/ddpg_routing/csv/{}/{}/r_pkt: No.{}, {}, {}, {}, {}, {}.csv"
                  .format(folder, noise_mode, ex, total_episodes, total_steps, noise_mode, mode, mode_flow_change),
                  header=False, index=False)

        df = pd.DataFrame(ep_converged)
        df.to_csv("/home/tud/Github/Master-Thesis/ddpg_routing/csv/{}/{}/CE: No.{}, {}, {}, {}, {}, {}.csv"
                  .format(folder, noise_mode, ex, total_episodes, total_steps, noise_mode, mode, mode_flow_change),
                  header=False, index=False)

        df = pd.DataFrame(reward_converged)
        df.to_csv("/home/tud/Github/Master-Thesis/ddpg_routing/csv/{}/{}/CR: No.{}, {}, {}, {}, {}, {}.csv"
                  .format(folder, noise_mode, ex, total_episodes, total_steps, noise_mode, mode, mode_flow_change),
                  header=False, index=False)

        df = pd.DataFrame(ep_converged2)
        df.to_csv("/home/tud/Github/Master-Thesis/ddpg_routing/csv/{}/{}/CE2: No.{}, {}, {}, {}, {}, {}.csv"
                  .format(folder, noise_mode, ex, total_episodes, total_steps, noise_mode, mode, mode_flow_change),
                  header=False, index=False)

        df = pd.DataFrame(reward_converged2)
        df.to_csv("/home/tud/Github/Master-Thesis/ddpg_routing/csv/{}/{}/CR2: No.{}, {}, {}, {}, {}, {}.csv"
                  .format(folder, noise_mode, ex, total_episodes, total_steps, noise_mode, mode, mode_flow_change),
                  header=False, index=False)
