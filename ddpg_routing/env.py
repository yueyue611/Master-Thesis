...
class Env:
    ...
    def new_flows(self, mode_flow_change, random_flows_change, changed_flows_num):
        if mode_flow_change == "JOIN":
            ...
            if random_flows_change:
                new_flows = self.get_flows(changed_flows_num)
            else:
                new_flows = [[0, 4, 10], [2, 0, 10]]
            ...
            if random_flows_change:
                ...
                new_flows = [[0, 4, 10], [0, 4, 10], [0, 4, 10]]
        else:
            new_flows = []
        return new_flows
    ...
    def observation_space(self):
        ...
        action_dim = self.total_switches ** 2 * self.action_mode
    ...
    def get_delay(self, state, optimal_path, queue_length_select):
        ...
        avg_delay = sum(cost2) / len(optimal_path)
